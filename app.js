...
const stageOptions = (selectedStageId) => state.stages.map((stage) => `<option value="${escapeHtml(stage.id)}" ${stage.id === selectedStageId ? 'selected' : ''}>${escapeHtml(stage.name)}</option>`).join('')
...
function renderFilters() {
  workspaceSwitcher.innerHTML = state.workspaces.map((workspace) => `<option value="${escapeHtml(workspace.id)}">${escapeHtml(workspace.name)}</option>`).join('')
  ...
  ownerFilter.innerHTML = owners.map((owner) => `<option value="${escapeHtml(owner)}">${owner === 'all' ? 'Todos os owners' : escapeHtml(owner)}</option>`).join('')
  ownerFilter.value = state.owner
  leadOwnerInput.innerHTML = ['<option value="">Sem owner</option>', ...activeMembers.map((member) => `<option value="${escapeHtml(member.name)}">${escapeHtml(member.name)}</option>`)].join('')
  const temperatures = ['all', 'hot', 'warm', 'cold']
  temperatureFilter.innerHTML = temperatures.map((value) => `<option value="${escapeHtml(value)}">${value === 'all' ? 'Todas as temperaturas' : escapeHtml(value)}</option>`).join('')
  ...
  statusFilter.innerHTML = statuses.map((value) => `<option value="${escapeHtml(value)}">${value === 'all' ? 'Todos os status' : escapeHtml(value)}</option>`).join('')
...
async function loadAnalytics() {
  try {
    ...
    sourcePerformance.innerHTML = payload.sources.map((item) => `<div class="stack-item"><div><strong>${escapeHtml(item.source)}</strong><p>${escapeHtml(item.count)} leads</p></div><span class="pill pill-success">${currency.format(item.revenue || 0)}</span></div>`).join('')
    ownerPerformance.innerHTML = payload.owners.map((item) => `<div class="stack-item"><div><strong>${escapeHtml(item.owner)}</strong><p>${escapeHtml(item.count)} leads</p></div><span class="pill pill-success">${currency.format(item.revenue || 0)}</span></div>`).join('')
    statusPerformance.innerHTML = payload.statuses.map((item) => `<div class="stack-item"><div><strong>${escapeHtml(item.status)}</strong><p>${escapeHtml(item.count)} leads</p></div><span class="pill pill-neutral">${currency.format(item.revenue || 0)}</span></div>`).join('')
  } catch (error) {
