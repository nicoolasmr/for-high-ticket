...
function renderFilters() {
  ...
  leadOwnerInput.innerHTML = ['<option value="">Sem owner</option>', ...activeMembers.map((member) => `<option value="${member.name}">${member.name}</option>`)].join('')
...
function renderInvites() {
  ...
  inviteList.innerHTML = state.invites.map((invite) => `<div class="stack-item"><div><strong>${invite.name}</strong><p>${invite.role} · ${invite.status}</p></div><button class="button button-primary small-button" data-accept-invite="${invite.id}">Aceitar</button></div>`).join('')
}
...
async function loadTeam() { state.team = (await request('/api/team')).items; teamList.innerHTML = state.team.map((member) => `<div class="stack-item"><div><strong>${member.name}</strong><p>${member.email} · ${member.role}</p></div><span class="pill ${member.status === 'active' ? 'pill-success' : 'pill-neutral'}">${member.status}</span></div>`).join(''); renderFilters() }
...
async function loadAnalytics() {
  try {
    ...
    insightsList.innerHTML = `<div class="stack-item"><div><strong>Acesso restrito</strong><p>${error.message}</p></div></div>`
    sourcePerformance.innerHTML = ''
    ownerPerformance.innerHTML = ''
    statusPerformance.innerHTML = ''
  }
