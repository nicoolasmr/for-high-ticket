...
async function loadDashboard() {
  ...
  kpiGrid.innerHTML = dashboard.kpis.map((kpi) => `<article class="metric-card glass-card compact-card"><span>${kpi.label}</span><strong>${typeof kpi.value === 'number' && kpi.label.includes('Receita') ? currency.format(kpi.value) : kpi.value}</strong><small>${kpi.detail}</small></article>`).join('')
  priorityPill.textContent = `${dashboard.priorities.length} leads críticos`
  priorityList.innerHTML = dashboard.priorities.map((lead) => `<button class="stack-item action-card" data-select-lead="${lead.id}"><div><strong>${lead.name}</strong><p>${lead.company} · ${lead.owner}</p></div><span class="pill pill-danger">${lead.lastReplyHours}h sem resposta</span></button>`).join('')
}
...
async function loadSummary(leadId) {
  ...
  aiSummary.innerHTML = `<div class="summary-block"><span class="eyebrow">${lead.name} · ${lead.company}</span><p>${lead.text}</p><p><strong>Status:</strong> ${lead.status}${lead.lostReason ? ` · ${lead.lostReason}` : ''}</p><div class="summary-columns"><div><strong>Objeções</strong><ul>${lead.objections.map((item) => `<li>${item}</li>`).join('')}</ul></div><div><strong>Sinais de compra</strong><ul>${lead.signals.map((item) => `<li>${item}</li>`).join('')}</ul></div></div><p><strong>Próxima melhor ação:</strong> ${lead.nextBestAction}</p><blockquote>${lead.suggestedReply}</blockquote></div>`
}

async function loadTimeline() {
  if (!state.selectedLeadId) return
  const payload = await request(`/api/leads/${state.selectedLeadId}/timeline`)
  timelineList.innerHTML = payload.items.map((event) => `<div class="stack-item vertical-item"><strong>${event.eventType}</strong><p>${JSON.stringify(event.payload)}</p><small>${new Date(event.createdAt).toLocaleString('pt-BR')}</small></div>`).join('')
}

async function loadNotes() {
  if (!state.selectedLeadId) return
  const payload = await request(`/api/leads/${state.selectedLeadId}/notes`)
  notesList.innerHTML = payload.items.map((note) => `<div class="stack-item vertical-item"><div><strong>${note.author}</strong><p>${note.body}</p></div><small>${new Date(note.created_at).toLocaleString('pt-BR')}</small></div>`).join('')
}

async function loadPipeline() {
  const payload = await request('/api/pipeline')
  pipelineBoard.innerHTML = payload.stages.map((stage) => `<section class="pipeline-column glass-card compact-card"><header><h3>${stage.name}</h3><span>${stage.leads.length}</span></header><div class="pipeline-column-body">${stage.leads.map((lead) => `<button class="pipeline-card" data-select-lead="${lead.id}"><div><strong>${lead.name}</strong><p>${lead.company}</p></div><small>${currency.format(lead.value)} · ${lead.owner}</small></button>`).join('')}</div></section>`).join('')
}

async function loadTasks() {
  const payload = await request('/api/tasks')
  completedCountPill.textContent = `${payload.completedCount} concluídas`
  taskList.innerHTML = payload.tasks.map((task) => `<div class="stack-item"><div><strong>${task.due_time}</strong><p>${task.title}</p><small>${task.lead_id ? `Lead: ${task.lead_id}` : 'Sem lead vinculado'}</small></div><div class="task-actions"><span class="pill ${task.priority === 'urgent' ? 'pill-danger' : 'pill-neutral'}">${task.priority}</span><button class="button button-ghost small-button" data-complete-task="${task.id}">Concluir</button></div></div>`).join('')
  onboardingList.innerHTML = payload.onboarding.map((step) => `<div class="stack-item"><div><strong>${step.title}</strong><p>${step.done ? 'Concluído' : 'Pendente para ativação'}</p></div><span class="pill ${step.done ? 'pill-success' : 'pill-neutral'}">${step.done ? 'feito' : 'pendente'}</span></div>`).join('')
}
